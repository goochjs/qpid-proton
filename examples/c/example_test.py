...
class Proc(Popen):
    """A example process that stores its output"""
...
# Work-around older python unittest.
...
class CExampleTest(BrokerTestCase):
    """Run the examples using the python broker."""
    # FIXME aconway 2016-07-06:
    broker_exe = [os.path.join("..", "cpp", "broker")]
#    broker_exe = [sys.executable, os.path.join(os.path.dirname(__file__), "..", "python", "broker.py")]

    def test_libuv_send(self):
        try:
            self.assertEqual("100 messages sent and acknowledged\n",
                             self.proc(["simple_send", "-a", self.addr]).wait_out())
        except NotFoundError, e:
            skipTest(str(e))

    # FIXME aconway 2016-07-06: more test automation
